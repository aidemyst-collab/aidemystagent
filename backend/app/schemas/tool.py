...
class DynamicMCPToolConfig(BaseModel):
    """
    Configuration for Dynamic MCP tools.

    These tools are created via UI and executed by the Dynamic MCP Server.
    Users define an API endpoint and parameters, and the MCP server handles
    the actual API calls with secure credential management.
    """
    # Tool identification
    name: str = Field(..., min_length=1, max_length=100, description="Tool name (used as MCP tool name)")
    description: str = Field(..., min_length=1, max_length=500, description="Tool description for LLM")

    # API configuration
    api_endpoint: str = Field(..., description="External API endpoint URL")
    method: HTTPMethod = Field(default=HTTPMethod.GET, description="HTTP method")

    # Parameters that the LLM can provide
    ...
    query_params: Optional[Dict[str, str]] = Field(None, description="Static query parameters")
    body_template: Optional[Dict[str, Any]] = Field(None, description="Body template for POST/PUT/PATCH")
    ...
    # Response handling
    response_path: Optional[str] = Field(None, description="JSON path to extract from response (e.g., 'data.items')")
    response_template: Optional[str] = Field(None, description="Template to format response")
...
class DynamicMCPToolCreate(BaseModel):
    """Schema for creating a Dynamic MCP tool."""
...
class DynamicMCPToolUpdate(BaseModel):
    """Schema for updating a Dynamic MCP tool."""
...
class DynamicMCPToolResponse(BaseModel):
    """Schema for Dynamic MCP tool response."""
    id: UUID
...
class DynamicMCPToolListResponse(BaseModel):
    """Schema for listing Dynamic MCP tools."""
